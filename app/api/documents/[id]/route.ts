...
import { getDocumentForUser, getDocumentById } from "@/lib/db";
...
export async function GET(
  ...
  const doc = isPractitioner
    ? await getDocumentById(params.id)
    : await getDocumentForUser(params.id, session.user.id as string);

  if (!doc) {
    // Either it doesn't exist, or (for clients) it belongs to someone
    // else — same response either way, so we don't leak which.
